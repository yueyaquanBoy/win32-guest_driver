: Param1=signWin7 | signVista | signWlh | signXP | signWXp | sign2K
: Param2=x86 | amd64
: Param3=path to INF file (SYS must be is in the same place) 
: Param4=version string to patch INF

if not SIGNCERT == "" set SIGNCERT=/f tools\NetKVMTemporaryCert.pfx /p password
...
if "%1"=="signWlh" goto signVista%USESIGNABILITY%
if "%1"=="signWnet" goto signXP%USESIGNABILITY%
if "%1"=="signXP" goto signXP%USESIGNABILITY%
if "%1"=="signWXp" goto signXP%USESIGNABILITY%
...
:_stampinf
REM Tools\xdate.exe -u > timestamp.txt
REM set /p STAMPINF_DATE= < timestamp.txt
REM del timestamp.txt
REM stampinf -f %1 -v %2 -a %_BUILDARCH%.%_NT_TARGET_MAJ_ARCH%.%_NT_TARGET_MIN_ARCH%
goto :eof
...
if exist %~dpn2.cat SignTool sign %SIGNCERT% %SIGNTIMESTAMP%  %~dpn2.cat 
if exist %~dpn2.sys SignTool sign %SIGNCERT% %SIGNTIMESTAMP%  %~dpn2.sys
goto :eof
